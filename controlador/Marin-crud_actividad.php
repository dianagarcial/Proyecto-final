...
require_once('../modelo/conexion.php');
	...
	foreach($select->fetchAll() as $AD){
		...
		public function ConsultarRechazado(){
			$db=Db::conectar();
			$listaActividadGen=[];
			$select=$db->query("SELECT ACTIVIDAD.CODIGO as codigo,GRUPO.codigo as codigoGrupo,GRUPO.codigo_Grup numGru,ASIGNATURA.codigo as codAsig, ASIGNATURA.nombre as Asignatura,(SELECT USUARIO.nombre FROM USUARIO JOIN PROFESOR ON USUARIO.nomUsuario= PROFESOR.usuario) as ProfesorNomb,\n"
			...
			. "WHERE RUBRICA.calificacion='Rechazado';");
		...
		foreach($select->fetchAll() as $AD){
			...
			public function ConsultarEntregado(){
				$db=Db::conectar();
				$listaActividadGen=[];
				$select=$db->query("SELECT ACTIVIDAD.CODIGO as codigo,GRUPO.codigo as codigoGrupo,GRUPO.codigo_Grup numGru,ASIGNATURA.codigo as codAsig, ASIGNATURA.nombre as Asignatura,(SELECT USUARIO.nombre FROM USUARIO JOIN PROFESOR ON USUARIO.nomUsuario= PROFESOR.usuario) as ProfesorNomb,\n"
			...
			foreach($select->fetchAll() as $AD){
				...
				public function ConsultarNoEntregado(){
					$db=Db::conectar();
					$listaActividadGen=[];
					$select=$db->query("SELECT ACTIVIDAD.CODIGO as codigo,GRUPO.codigo as codigoGrupo,GRUPO.codigo_Grup numGru,ASIGNATURA.codigo as codAsig, ASIGNATURA.nombre as Asignatura,(SELECT USUARIO.nombre FROM USUARIO JOIN PROFESOR ON USUARIO.nomUsuario= PROFESOR.usuario) as ProfesorNomb,\n"
					...
					. "on GRUPO.codigo_asgs=ASIGNATURA.codigo PROFESOR\n"
...
public function ConsultarAsignaturaEspecifica($id){
	$db=Db::conectar();
	$listaAsiganturaE=[];
	$select=$db->prepare( "SELECT iFNULL(ACTIVIDAD.fechaEntrega,'-') as fechaen,ASIGNATURA.nombre as asig,ASIGNATURA.codigo as codAsig, IFNULL(ACTIVIDAD.estado,'No entregado') as estado,ACTIVIDAD.codigo as idA, \n"
